...
> 当前状态：D01–D06 CPU modules 已完成；所有真实模型/GPU gates 与实验 runs 均 `NOT_STARTED`。本表是预注册队列，不是结果表。
...
|---|---|---:|---|---|
| CPU algorithm/framework | D01–D12 | 12 | core | 6 COMPLETE / 6 PLANNED |
...
核心进度分母为 `24`，当前 `6/24`；包含延后研究线的目录总数为 `32`。12 个学习课程章节不计入执行进度。完整定义以 `docs/planning/DEVELOPMENT_MODULES.md` 为准，当前下一模块是 D07。
