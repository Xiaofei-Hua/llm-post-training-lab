...
> 2026-09-09 更新；D11 CPU 首轮已完成，实测与负结果见 `docs/performance/CPU_BENCHMARK.md`。D13 tiny GPU 运行时的测量脚本已准备，因 GPU 被另一任务使用而暂停；D18–D20 真实模型测量仍待授权。
...
|---|---|---|---|---|---|---|---|
| D11 / Apple M3 Pro CPU / FP32 | rollout dense head → last-position head | 7.888 ms 完整 OPD step | 7.647 ms | median 约下降 3.1% | 轮间范围重叠；p95 未改善 | 采样 token 与 FP64 概率等价测试通过 | 不声称稳定加速；完整 kernel/RSS 负结果见 D11 报告 |
