...
- **Constraints**：D13 获得单 GPU tiny 模型验证授权后，用户因 GPU 正被其他任务使用而暂停实测；继续 CPU 与镜像环境准备，真实模型/数据与训练尚未授权；只使用公开数据和模型。
...
参考投入为算法 35%、框架 30%、性能 20%、指标分析 15%；用于排序任务，不建立工时审批。每个主要任务应回答一个具体技术问题，并交付代码、最小验证/测量和解释。D09–D12 的 CPU 模型接入、完整更新循环、profile 与学习实验已完成；D13 运行时代码与 CPU 测试已完成，GPU 实测暂停，尚不计入完成数。
